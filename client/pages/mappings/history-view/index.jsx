...
class EditPage extends React.Component {
    ...
    componentDidMount() {
        ...
        document.title = 'History (v' + this.props.match.params.version + ') of ' + this.props.match.params.template
            + '/' + this.props.match.params.lang + ' | DBpedia Mappings UI';
    }
        ...
        return (
            <section className="container">
                <div className="page-header">
                    ...
                    <h2 style={ { margin:0 } }>Viewing { ' v' + version} of <Link to={'/mappings/history/' +  title
                        + '/' + lang}>{title + ' (' +  lang + ')'}</Link></h2>

                </div>
                <div className="row">
                    <div className="col-sm-8">
                        <DetailsForm {...this.state.details}/>
                        {this.state.details.hydrated && <span>Last edited on
                        { Moment(this.state.details.edition.date).format('DD/MM/YYYY, HH:mm:ss') } by
                        { this.state.details.edition.username}</span>}
                        {this.state.details.hydrated && <span><br/>Edition comment: {this.state.details.oldComment}
                        </span>}
                    ...
                    <div className="col-sm-4">
                        <div className="panel panel-default">
                            ...
                            <div className="panel-body">
                                <b>Status when archived:  <span style={{ color: this.state.details.status.error ? 'red'
                                    : 'green' }}>{this.state.details.status.message}</span></b><br/>
                                <hr/>
                                { this.state.details.stats && Object.keys(this.state.details.stats).length > 0 && <div>
                                    ...
                                    <b>Mapped properties: </b> {this.state.details.stats.numMappedProperties}
                                         &nbsp;({this.getPercentage(this.state.details.stats.numMappedProperties,
                                         this.state.details.stats.numProperties)} %)<br/>
                                    <b>Property ocurrences: </b> {this.state.details.stats.numPropertyOcurrences}<br/>
                                    <b>Mapped property ocurrences: </b>
                                    {this.state.details.stats.numMappedPropertyOcurrences}
                                        &nbsp;({this.getPercentage(this.state.details.stats.numMappedPropertyOcurrences,
                                        this.state.details.stats.numPropertyOcurrences)} %)<br/>
                                    <b>Not found properties: </b> {this.state.details.stats.numPropertiesNotFound}
                                        &nbsp;({this.getPercentage(this.state.details.stats.numPropertiesNotFound,
                                        this.state.details.stats.numProperties)} %)<br/>
                                </div> }
                                { (!this.state.details.stats || Object.keys(this.state.details.stats).length === 0) &&
                                 <div>
